...
Like the ConfigMap `custom-http-errors`, this annotation will set NGINX `proxy-intercept-errors`, but only for the hostname associated with this ingress. Different ingresses can specify different sets of error codes. If multiple ingress objects share the same hostname, this annotation can be used to intercept different error codes for each hostname. If `custom-http-errors` is also specified globally, the error values specified in this annotation will override the global value for this ingress' hostname and path.
